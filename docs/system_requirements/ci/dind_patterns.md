...
docker run --rm \
  ...
  sh -c "shards install && crystal spec"
```
...
services:
  tests:
    ...
    command: sh -c "shards install && crystal spec"
```
...
Testcontainers for Crystal reads `DOCKER_HOST` as the first step in [socket detection](../index.md).
